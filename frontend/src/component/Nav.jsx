      ...
      <div className='w-[100%] h-[70px] fixed top-0 px-[20px] py=-[10px] flex items-center justify-between bg-[#00000047] z-10'>
        ...
        <div className={`fixed top-0 left-0 w-[100vw] h-[100vh] bg-[#000000d6] flex items-center justify-center flex-col gap-5 z-10 lg:hidden ${showHam ? "translate-x-[0] transition duration-600": "translate-x-[-100%] transition duration-600"}`}>
         <RxCross2 className='w-[35px] h-[35px] fill-white absolute top-5 right-[4%]' onClick={()=> setShowHam(prev=> !prev)} />
            ...
            {userData?.photoUrl?<img src={userData?.photoUrl} className='w-[50px] h-[50px] rounded-full text-white flex items-center justify-center text-[20px] border-2 bg-[black] border-white cursor-pointer' onClick={() => setShow(prev=>!prev)}/> :<div className='w-[50px] h-[50px] rounded-full text-white flex items-center justify-center text-[20px] border-2 bg-[black] border-white cursor-pointer'>
            {userData?.name?.slice(0,1).toUpperCase()}
            </div>}
             ...
             <div className='w-[200px] h-[65px] border-2 border-white text-white bg-[black] rounded-[10px] flex items-center justify-center text-[18px] font-light cursor-pointer' onClick={() => navigate('/mycourses')}>My Courses</div>
